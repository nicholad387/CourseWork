...
(*PARTNER 1: Evelyn Torres
  PARTNER 2: Nicholas Mohamed
...
and
  ...
  | Proj(e,id) ->
    ...
    (match List.assoc_opt id l with 
      | None -> error "error"
      | Some (false, x)  -> return x
      | Some (true, RefVal(x)) ->  Store.deref g_store x
      | _ -> error "Field not mutable" )

  | SetField(e1,id,e2) ->
      eval_expr e1 >>=
      fields_of_recordVal >>= fun h ->
      (*eval_expr e2 >>=*)
      (match List.assoc_opt id h with 
        |None -> error "error"
        |Some(false, x) -> error "field not mutable"
        |Some (true, RefVal x) -> 
          eval_expr e2 >>= 
          Store.set_ref g_store x >>= 
          fun _ -> 
          return UnitVal
        |Some(false, _) -> error "Nothing"
        |Some(true, _) -> error "Nothing"
      )

(*true with nothing and flase with something or nothing *)
  | IsNumber(e) ->
     eval_expr e >>= 
     fun l ->
     (match l with 
