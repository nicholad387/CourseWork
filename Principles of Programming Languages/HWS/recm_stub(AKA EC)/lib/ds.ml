...
let rec rdecs_helper rdecs id = 
  match rdecs with
    |[] -> None
    |(v,par,_,_,body)::rest->
      if id = v
        then Some(v,par,body)
      else rdecs_helper rest id
...
let rec apply_env : string -> exp_val ea_result =
  ...
  | ExtendEnvRec(rdecs, tail) -> 
    let dec = (rdecs_helper rdecs id) in
    match dec with
    | None -> apply_env id tail
    | Some(v,par,body)-> Ok(ProcVal (par, body, env))




(**Make helper to go through rdecs, if rdecs is empty then apply_env, 
if it does have something then extend it with the environment *)                                        
