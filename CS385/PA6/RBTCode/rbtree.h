/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Nicholas Mohamed & Nerissa Lundquist
 * Version     : 1.0
...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        if (find(key) != end()) {
        	...
        	string s = ss.str();
        	delete z;
        	throw tree_exception("Attempt to insert duplicate key '" + s + "'.");
        }
        while (x != nullptr) {
        	y = x;
...
private:
    ...
    void insert_fixup(Node<K, V> *z) {
        ...
        while (z -> parent != nullptr) {
            ...
            if (z->parent->parent->left != nullptr && z -> parent->key() == z -> parent -> parent -> left->key()) {
     ...
     */
    void left_rotate(Node<K, V> *x) {   // written from psudo code from CLRS Chapter 13
        // TODO
        Node<K, V> *y = x -> right;
        x -> right = y -> left;
     ...
     */
    void right_rotate(Node<K, V> *x) {      // just the inverse of left shift
        // TODO
        Node<K, V> *y = x -> left;
        x -> left = y -> right;
    ...
    int diameter(Node<K, V> *node) const {
        ...
        return max(2 + height(node -> left) + height(node -> right), max(diameter(node->left), diameter(node->right)));
    }
